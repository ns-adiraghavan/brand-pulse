...
const SECTION_META: Record<string, { icon: string; subtitle: string }> = {
  performance: {
    icon: "📊",
    subtitle: "Where are we winning or losing in the funnel vs competitors?",
  },
  perception: {
    icon: "🧠",
    subtitle: "What does each brand stand for in the customer's mind?",
  },
  campaign: {
    icon: "📣",
    subtitle: "Are our marketing efforts translating into brand lift?",
  },
  behavior: {
    icon: "🛒",
    subtitle: "How do customers discover and engage with our category?",
  },
  customer: {
    icon: "👥",
    subtitle: "Who are our users and what drives their decisions?",
  },
  deepdive: {
    icon: "🔬",
    subtitle: "Which segments are driving or hurting our funnel performance?",
  },
};
...
function SectionHeading({ id, label }: { id: string; label: string }) {
  const meta = SECTION_META[id] ?? { icon: "📋", subtitle: "" };
  return (
    <div className="mb-5">
    ...
    <section id={id} className="scroll-mt-20">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-3">
        {previewStats.map((stat) => (
          <div key={stat.label} className="flex flex-col gap-2 rounded-xl border border-border-dim bg-surface p-5">
...
const SectionCard = ({ id, label }: SectionCardProps) => {
  if (id === "performance") return (
    <section id="performance" className="scroll-mt-20">
      <SectionHeading id="performance" label="Performance" />
      <PerformanceSection />
    </section>
  );

  if (id === "perception") return (
    <section id="perception" className="scroll-mt-20">
      <SectionHeading id="perception" label="Perception" />
      <PerceptionSection />
    </section>
  );
  ...
  if (id === "campaign") return (
    <section id="campaign" className="scroll-mt-20">
      <SectionHeading id="campaign" label="Campaign" />
      <CampaignSection />
    </section>
  );
  ...
  if (id === "behavior") return (
    <section id="behavior" className="scroll-mt-20">
      <SectionHeading id="behavior" label="Category & Behaviour" />
      <BehaviorSection />
    </section>
  );
  ...
  if (id === "customer") return (
    <section id="customer" className="scroll-mt-20">
      <SectionHeading id="customer" label="Customer Understanding" />
      <CustomerSection />
    </section>
  );
  ...
  if (id === "deepdive") return (
    <section id="deepdive" className="scroll-mt-20">
      <SectionHeading id="deepdive" label="Funnel Deep Dive" />
      <DeepDiveSection />
    </section>
  );
